...
import { z } from 'zod';
import { MatchStatus } from '@prisma/client'; // Import the Enum type
...
export async function POST(
    ...
    try {
        ...
        const { teamId: homeTeamId } = params; // Rename teamId to homeTeamId for clarity
        
        // 1. Authorization: Check user role and team management
        if (!session || !session.user || session.user.role !== REQUIRED_ROLE) {
            return new NextResponse('Forbidden: Only Coaches can schedule games.', { status: 403 });
        }
        ...
        const { opponentTeamId: awayTeamId, scheduledTime, venue, isHomeGame } = validatedData.data;
