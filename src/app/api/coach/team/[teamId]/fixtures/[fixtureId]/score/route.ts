  ...
  try {
    ...
    if (!session || !session.user || session.user.role !== REQUIRED_ROLE) {
      return new NextResponse(
        "Forbidden: Only Coaches can update match scores.",
        { status: 403 },
      );
    }
    ...
    if (
      ...
      return new NextResponse(
        "Forbidden: You can only update scores for your own fixtures.",
        { status: 403 },
      );
    }
    ...
    if (fixture.status === MatchStatus.COMPLETED) {
      return new NextResponse(
        "Conflict: This fixture is already marked as COMPLETED.",
        { status: 409 },
      );
    }
    ...
    const [updatedFixture, homeStatsUpdate, awayStatsUpdate] =
    ...
    return new NextResponse(
      "Internal Server Error while finalizing match score and stats.",
      { status: 500 },
    );
  }
}