...
import { z } from 'zod';
import { Prisma } from '@prisma/client'; // ✅ CORRECTED IMPORT: Using Prisma namespace
...
const CreateFixtureSchema = z.object({
    ...
    scheduledTime: z.string().datetime({ message: "Must be a valid ISO date string." }), 
...
async function authorizeCoach(teamId: string) {
    ...
    if (session.user.role !== REQUIRED_ROLE) {
        return { authorized: false, status: 403, message: 'Forbidden: Only Coaches can view/manage fixtures.' };
    }
...
export async function GET(
    ...
    try {
        ...
        const fixtures = await db.fixture.findMany({
            ...
            select: {
                id: true, scheduledTime: true, venue: true, status: true, homeScore: true, awayScore: true,
        ...
        const formattedFixtures = fixtures.map(fixture => {
            const isHomeGame = fixture.homeTeamId === myTeamId;
            ...
            return {
                ...
                score: fixture.status === MatchStatus.COMPLETED 
        ...
        return new NextResponse('Internal Server Error while fetching fixtures.', { status: 500 });
    }
}
...
export async function POST(
    ...
    try {
        ...
        const { opponentTeamId: otherTeamId, scheduledTime, venue, isHomeGame } = validatedData.data;
        
        if (homeTeamId === otherTeamId) {
            return new NextResponse('Bad Request: Cannot schedule a fixture against your own team.', { status: 400 });
        }
        ...
        const newFixture = await db.fixture.create({
            ...
            select: { id: true, scheduledTime: true, venue: true, homeTeam: { select: { name: true } }, awayTeam: { select: { name: true } } }
        });
        ...
        return new NextResponse('Internal Server Error while scheduling fixture.', { status: 500 });
    }
}