...
import { LogType, MatchStatus } from '@prisma/client';
...
export async function POST(
    ...
    try {
        ...
        if (!session || !session.user || session.user.role !== REQUIRED_ROLE) {
            return new NextResponse('Forbidden: Only Coaches can log match events.', { status: 403 });
        }
        ...
        if (fixture.status !== MatchStatus.LIVE) {
            return new NextResponse(`Match is not live: ${fixture.status}`, { status: 400 });
        }
        ...
        if (player.teamId !== coachTeamId) {
            return new NextResponse('Forbidden: You can only log events for your own team.', { status: 403 });
        }
        ...
        const matchLog = await db.matchLog.create({
            data: {
                fixtureId,
                logEntry: detail || `${eventType} for player ${playerId} at minute ${minute}.`,
            }
        });
        ...
        const [playerLog] = await db.$transaction([
            ...
            db.playerStats.update({
                ...
                data: {
                    ...
                    yellowCards: eventType === LogType.YELLOW_CARD ? { increment: 1 } : undefined,
                    redCards: eventType === LogType.RED_CARD ? { increment: 1 } : undefined,
                }
            })
