...
import { z } from 'zod';
import * as Prisma from '@prisma/client'; // Use wildcard import for robust access
...
function calculateStats(homeScore: number, awayScore: number) {
    ...
    return {
        home: { ...homeStats, goalsFor: homeGoalsFor, goalsAgainst: homeGoalsAgainst, goalDifference: homeGoalsFor - homeGoalsAgainst },
        away: { ...awayStats, goalsFor: awayScore, goalsAgainst: homeScore, goalDifference: awayScore - homeScore },
    };
}
...
export const PATCH = async (
    ...
    try {
        ...
        if (!session || session.user.role !== REQUIRED_ROLE || !session.user.managedTeamId) {
        ...
        if (!validatedData.success) {
            return new NextResponse(`Invalid input: ${validatedData.error.errors[0]?.message}`, { status: 400 });
        }
        ...
        if (fixture.status === MatchStatus.COMPLETED) {
            return new NextResponse('Conflict: Match already completed.', { status: 409 });
        }
        ...
        const [updatedFixture] = await db.$transaction([
            ...
            db.teamStats.update({
                ...
                data: {
                    wins: { increment: stats.home.wins }, losses: { increment: stats.home.losses }, draws: { increment: stats.home.draws }, points: { increment: stats.home.points }, goalsFor: { increment: stats.home.goalsFor }, goalsAgainst: { increment: stats.home.goalsAgainst }, goalDifference: { increment: stats.home.goalDifference },
                },
            }),
            db.teamStats.update({
                where: { teamId: fixture.awayTeamId },
                data: {
                    wins: { increment: stats.away.wins }, losses: { increment: stats.away.losses }, draws: { increment: stats.away.draws }, points: { increment: stats.away.points }, goalsFor: { increment: stats.away.goalsFor }, goalsAgainst: { increment: stats.away.goalsAgainst }, goalDifference: { increment: stats.away.goalDifference },
                },
            }),
        ]);

        return NextResponse.json({ message: `Fixture ${fixtureId} finalized.`, fixture: updatedFixture }, { status: 200 });
